...
void process_promisc(void *buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  if (frame->version() == 0 && frame->type() == WIFI_PKT_DATA && frame->subtype() == 0x00 && 
    ...
    uint16_t payload_length = pkt->rx_ctrl.sig_len - sizeof(wifi_ieee80211_data)- 4;
    Serial.print("Payload data: ");
    for (int i = 0; i < payload_length; i++) {
      Serial.print(payload_data[i], HEX);
      Serial.print(" ");
    }
    Serial.println();
    ...
    // Print the payload to see what's in there
    // Access the payload data
   
    //uint8_t *payload = pkt->payload;
    //int payload_length = pkt->rx_ctrl.sig_len - sizeof(wifi_pkt_rx_ctrl_t);
